...
def get_metadata(file_path):
    try:
        ...
        print(
            colored(f"\nGot metadata ", "light_green", "on_black"),
            f"for {audio['title']}.",
        )
        ...
        print(
            colored("Error getting metadata", "light_red", "on_black"),
            f" for {file_path}: {e}",
        )
            ...
            if any(file.endswith(ext) for ext in audio_formats):
                ...
                if artist and album and year:
                    ...
                    destination_folder = os.path.join(
                        source_folder, f"{artist} - {album} - ({year})"
                    )  # makes the folder
                    ...
                    try:
                        ...
                        print(
                            colored("Successfully moved ", "green", "on_black")
...
if __name__ == "__main__":
    ...
    main_folder = str(
        input(
            "Enter the path to the folder containing the music you want to organize: "
        )
    )
    ...
    print(
        "Please report any issues on GitHub: https://github.com/vb2007/music-sorter/issues/new"
    )
    input()
